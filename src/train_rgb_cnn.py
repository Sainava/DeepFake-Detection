...
# === CONFIG ===
DATA_DIR = os.path.join(".", "dataset", "rgb")  # use your RGB frames
SAVE_DIR = os.path.join(".", "models", "rgb_cnn")
...
def train(model, train_dl, val_dl, optimizer, criterion, scheduler):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            print(f"[INFO] Best RGB model saved at epoch {epoch+1}")
            patience_counter = 0
        else:
            patience_counter += 1
...
def evaluate(model, test_dl):
    ...
    print(f"[TEST] RGB Acc: {acc:.4f}")

# === MAIN ===
if __name__ == "__main__":
    print("[INFO] Preparing RGB data...")
    grouped = get_samples(DATA_DIR)
    train_samples, val_samples, test_samples = stratified_split(grouped, VAL_SPLIT, TEST_SPLIT)
    train_ds = RGBDataset(train_samples, transform=train_transform)
    val_ds   = RGBDataset(val_samples, transform=eval_transform)
    test_ds  = RGBDataset(test_samples, transform=eval_transform)
    ...
    print("[INFO] Evaluating best RGB model...")
